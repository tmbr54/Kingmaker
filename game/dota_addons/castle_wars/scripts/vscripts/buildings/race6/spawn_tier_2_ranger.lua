...
function spawn_unit(event)
	...
	if RemoveGold(iPlayerID, 12, 7) == true then
		if currentPop + iFoodPerSpawn <= populationLimit then
			local spawned_unit = CreateUnitByName("race_6_range_tier_2_viper", event.caster:GetAbsOrigin(), true, hero, hero, iTeamNumber)
