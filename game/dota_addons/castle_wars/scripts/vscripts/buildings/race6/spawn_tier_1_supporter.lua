...
function spawn_unit(event)
	...
	if RemoveGold(iPlayerID, 10, 7) == true then
		if currentPop + iFoodPerSpawn <= populationLimit then
			local spawned_unit = CreateUnitByName("race_6_special_tier_1_succubus", event.caster:GetAbsOrigin(), true, hero, hero, iTeamNumber)
			...
			ParticleManager:SetParticleControl(timerParticle, 10, Vector(0.6, 0.045, 0.37))
