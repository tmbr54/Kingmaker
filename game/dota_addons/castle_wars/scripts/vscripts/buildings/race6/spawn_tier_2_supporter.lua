...
function spawn_unit(event)
	...
	if RemoveGold(iPlayerID, 24, 7) == true then
		if currentPop + iFoodPerSpawn <= populationLimit then
			local spawned_unit = CreateUnitByName("race_6_special_tier_2_succubus", event.caster:GetAbsOrigin(), true, hero, hero, iTeamNumber)
