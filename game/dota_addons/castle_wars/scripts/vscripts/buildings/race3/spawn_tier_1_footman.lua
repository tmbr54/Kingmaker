...
function spawn_unit(event)
	...
	if RemoveGold(iPlayerID, 2, 7) == true then
		if currentPop + iFoodPerSpawn <= populationLimit then
			local spawned_unit = CreateUnitByName("race_3_melee_tier_1_footman", event.caster:GetAbsOrigin(), true, hero, hero, iTeamNumber)
