...
function spawn_unit(event)
	...
	if RemoveGold(iPlayerID, 10, 7) == true then
		if currentPop + iFoodPerSpawn <= populationLimit then
			local spawned_unit = CreateUnitByName("race_3_special_tier_1_shaman", event.caster:GetAbsOrigin(), true, hPlayer, hPlayer, iTeamNumber)
			...
			ParticleManager:SetParticleControl(timerParticle, 10, Vector(0.86, 0.06, 0.83))
