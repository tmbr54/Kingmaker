...
function spawn_unit(event)
	...
	if RemoveGold(iPlayerID, 24, 7) == true then
		if currentPop + iFoodPerSpawn <= populationLimit then
			local spawned_unit = CreateUnitByName("race_7_special_tier_2_morph", event.caster:GetAbsOrigin(), true, hero, hero, iTeamNumber)
			...
			ParticleManager:SetParticleControl(timerParticle, 10, Vector(0.2, 0.6, 1))
